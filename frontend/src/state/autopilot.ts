import { api } from '../api'
import type { OutboxEvent, ServerIssue, ServerRecommendation } from '../types'
 ...
 */
export type ApData = { mission: string; events: OutboxEvent[]; issues: ServerIssue[]; recs: ServerRecommendation[]; loadedAt: number; error?: string }
...
async function fetch(e: Entry): Promise<void> {
  ...
  e.inflight = true
  try {
    const [mission, events, issues, recs] = await Promise.all([
      ...
      api.helper.recommendations(e.serverId, e.projectId)
    ])
    const dir = e.dir.replace(/\/$/, '')
    e.data = { mission, events: events.filter((ev) => !dir || ev.project === dir || ev.project === e.dir), issues, recs, loadedAt: Date.now() }
  } catch (err) {
...
export const autopilot = {
  /** Registers a project for background syncing (idempotent) and returns its data. */
    ...
    let e = entries.get(k)
    if (!e) {
      e = { serverId, projectId, dir, data: { mission: '', events: [], issues: [], recs: [], loadedAt: 0 }, subs: new Set(), timer: null, inflight: false }
      entries.set(k, e)
      void fetch(e)
    } else if (e.dir !== dir) {
      e.dir = dir
  ...
  push(serverId: string, ev: OutboxEvent): void {
    for (const e of entries.values()) {
      ...
      e.subs.forEach((cb) => cb())
      if (ev.type === 'issue' || ev.type === 'issue-update' || ev.type === 'recommendation' || ev.type === 'rec-update') void fetch(e)
    }
  }
}
