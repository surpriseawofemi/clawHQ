...
func (s *ServerService) OpenShellIn(ctx context.Context, id, dir string, cols, rows int, session string) (string, error) {
	...
	if session != "" && !p.TmuxOff {
		...
		command = "bash -lc " + shq(cd+"if command -v tmux >/dev/null 2>&1; then exec tmux -u new-session -A -s "+shq(name)+" \\; set -g mouse on \\; set -g history-limit 20000 \\; set -g status off \\; set -s set-clipboard on; else exec bash -l; fi")
	}
